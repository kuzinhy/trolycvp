...
export const KnowledgeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const syncGoogleContacts = useCallback(async () => {
    ...
    try {
      if (!googleDriveToken) {
        showToast("Đang kết nối Google Contacts...", "info");
        await signInWithGoogle(true);
        return showToast("Đã xác thực Google Contacts, vui lòng ấn lại nút Đồng bộ (tải lại trang nếu cần).", "success");
      }
      
      showToast("Đang tải danh bạ từ Google Contacts...", "info");
      
      const res = await fetch(`https://people.googleapis.com/v1/people/me/connections?personFields=names,emailAddresses,organizations,phoneNumbers`, {
        headers: { Authorization: `Bearer ${googleDriveToken}` },
      });
      const data = await res.json();
      ...
      if (data.connections && data.connections.length > 0) {
        ...
        for (const person of data.connections) {
          ...
          const contactEntry = `Danh bạ: ${name}`;
          const isDuplicate = aiKnowledge.some(k => k.title === contactEntry);
          if (!isDuplicate) {
            try {
              let textContent = `Thông tin liên lạc:\n- Tên: ${name}\n`;
              if (email) textContent += `- Email: ${email}\n`;
              if (phone) textContent += `- Số điện thoại: ${phone}\n`;
              if (organization) textContent += `- Tổ chức/Cơ quan: ${organization}\n`;
              if (title) textContent += `- Chức vụ: ${title}\n`;

              await processAndSaveKnowledge(`${contactEntry}\n${textContent}`, ['contacts-sync', 'nhân sự'], 'Nhân sự - Tổ chức');
              count++;
            } catch (err) {
              console.warn("Could not process contact", err);
            }
          }
        }
        showToast(`Đã đồng bộ ${count} danh bạ vào bộ não.`, "success");
      } else {
        showToast("Không tìm thấy danh bạ.", "info");
      }
    } catch (e) {
       console.error("Contacts error", e);
       showToast("Trải qua lỗi xác thực danh bạ. Vui lòng đăng nhập lại.", "error");     
    }
  }, [user, googleDriveToken, signInWithGoogle, aiKnowledge, showToast, processAndSaveKnowledge]);
