...
export function useChat(
  ...
  const triggerAutoLearning = useCallback(async (history: Message[], isManual: boolean = false) => {
    ...
    try {
      ...
      console.warn("Auto-learning background warning (quota/rate-limit/network):", e);
  ...
  const handleSend = useCallback(async (textInput?: string | React.MouseEvent | React.KeyboardEvent, fileContent?: string) => {
    ...
    try {
      ...
      userName = user?.displayName || user?.email?.split('@')[0] || 'Đồng chí';
...
`;
      ...
      dynamicInstruction = SYSTEM_INSTRUCTION
      ...
      if (aiText.trim().length < 5) {
        try {
          console.log("[useChat] Stream failed, attempting non-stream fallback with gemini-3.7-flash...");
          const nonStreamRes = await generateContentWithRetry({
            model: "gemini-3.7-flash",
            contents: [{
              role: 'user',
              parts: [{
                text: `${dynamicInstruction}\n\n${scheduleContext}\n\nDỮ LIỆU TRI THỨC BỔ TRỢ (KNOWLEDGE BASE):\n${knowledgeContext}\n\nLỊCH SỬ HỘI THOẠI GẦN ĐÂY:\n${messages.slice(-10).map(m => `${m.role === 'user' ? 'Người dùng' : 'AI'}: ${m.content}`).join('\n')}\n\nYÊU CẦU HIỆN TẠI TỪ ${userName.toUpperCase()}: ${userMessage.content}`
              }]
            }],
          ...
          console.warn("[useChat] Non-stream fallback also failed:", retryErr);
        }
