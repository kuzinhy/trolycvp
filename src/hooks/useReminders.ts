  ...
  useEffect(() => {
    const checkReminders = () => {
      ...
      birthdays.forEach(birthday => {
        ...
        const endOfBirthday = new Date(currentYear, month, day, 23, 59, 59);
        const reminderId = `birthday-${birthday.id}-${currentYear}-${reminderType}-${reminderValue}`;
        
        if (now >= reminderTime && now <= endOfBirthday && !processedReminders.current.has(reminderId)) {
          const todayDate = new Date(now.getFullYear(), now.getMonth(), now.getDate());
          const targetBDate = new Date(currentYear, month, day);
          
          let desc = '';
          if (todayDate.getTime() === targetBDate.getTime()) {
            desc = `Hôm nay là sinh nhật của đồng chí ${birthday.name} (${birthday.date}). Hãy gửi lời chúc mừng tốt đẹp nhất!`;
          } else if (targetBDate.getTime() - todayDate.getTime() === 86400000) {
            desc = `Ngày mai là sinh nhật của đồng chí ${birthday.name} (${birthday.date}). Đồng chí hãy chuẩn bị lời chúc hoặc quà tặng cần thiết.`;
          } else {
            desc = `Sắp tới sinh nhật của đồng chí ${birthday.name} (${birthday.date}) vào ngày ${birthday.date}.`;
          }
