...
import { collection, query, orderBy, onSnapshot, addDoc, serverTimestamp, doc, updateDoc, writeBatch, where, arrayUnion, getDocFromServer, limit, setDoc } from 'firebase/firestore';
...
export function useNotifications() {
  ...
  const addNotification = useCallback(async (notification: Omit<Notification, 'id' | 'timestamp' | 'isRead'>, targetUserId: string = 'all', targetUnitId?: string, customId?: string) => {
    ...
    try {
      ...
      if (customId) {
        await setDoc(doc(db, 'notifications', customId), payload, { merge: true });
      } else {
