...
export const AssignmentTracking: React.FC<AssignmentTrackingProps> = ({ tasks: systemTasks, isAdmin = false }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      let allTasks = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })) as ViceChiefTask[];
      
      if (allTasks.length === 0) {
        const defaultViceTasks: Omit<ViceChiefTask, 'id'>[] = [
          // Phó Chánh Văn phòng 1 (Lê Thị Kiều Oanh)
          {
            title: "Thẩm tra lý lịch và hoàn thiện hồ sơ kết nạp 05 Đảng viên mới đợt 2/9",
            description: "Phối hợp với Chi bộ khu phố và cấp ủy cơ sở hoàn tất thủ tục xác minh 3 đời.",
            deadline: "2026-08-25",
            importance: "high",
            progress: 70,
            viceChiefId: 1,
          ...
          {
            title: "Tổng hợp báo cáo công tác Tuyên giáo và nắm bắt dư luận xã hội tháng 8/2026",
            description: "Báo cáo tình hình tư tưởng cán bộ, đảng viên và nhân dân trên địa bàn.",
            deadline: "2026-08-28",
            importance: "medium",
            progress: 85,
            viceChiefId: 1,
          ...
          {
            title: "Rà soát, chuẩn bị danh sách khen thưởng tổ chức Đảng và Đảng viên tiêu biểu",
            description: "Lập danh sách các tập thể, cá nhân hoàn thành xuất sắc nhiệm vụ năm 2026.",
            deadline: "2026-09-05",
            importance: "medium",
            progress: 40,
            viceChiefId: 1,
          ...
          // Phó Chánh Văn phòng 2 (Trần Quốc Bảo)
          {
            title: "Kiểm tra việc thực hiện Kết luận của Thường trực Đảng ủy tại 03 Chi bộ trực thuộc",
            description: "Đánh giá tiến độ giải quyết các kiến nghị cử tri và thực hiện chỉ tiêu nghị quyết.",
            deadline: "2026-08-22",
            importance: "high",
            progress: 60,
            viceChiefId: 2,
          ...
          {
            title: "Hoàn thành dự thảo Báo cáo sơ kết công tác kiểm tra, giám sát 6 tháng đầu năm",
            description: "Đánh giá các cuộc kiểm tra chuyên đề và việc chấp hành quy chế làm việc.",
            deadline: "2026-08-20",
            importance: "high",
            progress: 90,
            viceChiefId: 2,
          ...
          {
            title: "Xây dựng phương án đảm bảo an toàn thông tin và an ninh mạng Văn phòng Đảng ủy",
            description: "Rà soát quy trình quản lý tài liệu mật và phần mềm chỉ huy tác chiến.",
            deadline: "2026-08-26",
            importance: "critical",
            progress: 75,
            viceChiefId: 2,
        ...
        try {
          const addedTasks: ViceChiefTask[] = [];
          for (const t of defaultViceTasks) {
            const docRef = await addDoc(collection(db, 'vice_chief_tasks'), t);
            addedTasks.push({ id: docRef.id, ...t });
          }
          allTasks = addedTasks;
        } catch (e) {
          console.error("Error creating default vice chief tasks:", e);
          allTasks = defaultViceTasks.map((t, idx) => ({ id: `default-${idx}`, ...t }));
        }
      }
