  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (generationMode === 'auto') {
        if (reportType === 'document_summary') {
          // Báo cáo từ tài liệu tham khảo
          const recentDocs = knowledge.slice(0, 5); // Lấy 5 tài liệu mới nhất
          const docsContext = recentDocs.map((doc, i) => `Tài liệu ${i + 1}: ${doc.title}\nNội dung chính: ${doc.content?.substring(0, 500)}...`).join('\n\n');
          
          prompt = `
Bạn là môt trợ lý văn phòng Đảng ủy thông minh. Hãy tổng hợp và sinh một Báo Cáo Tóm Tắt Thông Minh dựa trên các tài liệu tham khảo gần đây nhất trong hệ thống.

**Danh sách Tài liệu tham khảo:**
${docsContext || 'Không có tài liệu nào gần đây.'}

**Yêu cầu báo cáo:**
- Tiêu đề: BÁO CÁO TỔNG HỢP NỘI DUNG TỪ TÀI LIỆU THAM KHẢO
- Nhận diện các điểm mấu chốt, chỉ đạo quan trọng từ các tài liệu trên.
- Sử dụng tiếng Việt trang trọng, phong cách chuyên nghiệp.
- Cấu trúc:
  I. TỔNG QUAN TÀI LIỆU
  II. NỘI DUNG CHỈ ĐẠO TRỌNG TÂM
  III. NHIỆM VỤ ĐẶT RA
  IV. KẾT LUẬN & ĐỀ XUẤT
Trình bày bằng format Markdown ngăn nắp.
`;
          ...
          {generationMode === 'auto' ? (
            <div className="flex items-center gap-4">
              ...
              >
                ...
                <option value="document_summary">Tóm tắt từ tài liệu tham khảo</option>
              </select>
            </div>
          ) : (
