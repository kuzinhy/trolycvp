...
export const ChatHistoryModule: React.FC<ChatHistoryModuleProps> = ({ 
  ...
  const handleShare = async (chat: any) => {
    ...
    if (navigator.share) {
      ...
      showToast('Đã sao chép nội dung vào bộ nhớ tạm để chia sẻ.', 'success');
    }
  };
  ...
  const confirmClearAll = async () => {
    ...
    try {
      ...
      showToast('Đã dọn dẹp sạch toàn bộ lịch sử trò chuyện thành công.', 'success');
    } catch (e) {
      console.error(e);
      showToast('Có lỗi xảy ra khi dọn dẹp lịch sử.', 'error');
    } finally {
      setIsClearing(false);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 h-[calc(100vh-80px)] p-2">
      ...
      <ConfirmationModal
        ...
        message="Đồng chí có chắc chắn muốn xóa toàn bộ lịch sử hội thoại? Hành động này không thể hoàn tác và toàn bộ dữ liệu trò chuyện cục bộ sẽ bị xóa."
