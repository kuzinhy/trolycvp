...
export const QuickNotes: React.FC<{ 
  ...
  const { user } = useAuth();
  const storageKey = `ai_assistant_quick_notes_${user?.uid || 'guest'}`;
  ...
  const confirmDelete = () => {
    ...
    saveNotes(notes.filter(n => n.id !== noteToDelete));
  ...
  return (
    ...
    >
      ...
      <ConfirmationModal
        ...
        title="Xóa ghi chú"
        message="Đồng chí có chắc chắn muốn xóa ghi chú này khỏi sổ tay cá nhân? Hành động này không thể hoàn tác."
