...
export const EliteCommandCenter: React.FC<EliteCommandCenterProps> = ({ 
  ...
  // Lọc nhiệm vụ quá hạn hoặc sắp đến hạn (trực quan hóa ngữ cảnh thời gian thực)
  const overdueTasks = React.useMemo(() => {
    if (!tasks) return [];
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const inThreeDays = new Date(today);
    inThreeDays.setDate(today.getDate() + 2); // Hôm nay, ngày mai, ngày kia
  ...
  const dynamicCommandGroups = React.useMemo(() => {
    const groups: any[] = [];
    
    if (overdueTasks.length > 0) {
      groups.push({
        title: "🔥 Đề xuất khẩn cấp (Hạn chót)",
        items: overdueTasks.map(task => ({
          id: `focus-task-${task.id}`,
          label: `Ưu tiên xử lý ngay: "${task.title}" (Hạn chót: ${task.deadline})`,
          icon: <Zap size={16} className="text-amber-500 animate-pulse" />,
          action: 'focus-task',
          taskData: task
        }))
      });
  ...
  const filteredGroups = query ? dynamicCommandGroups.map(group => ({
    ...group,
    items: group.items.filter(item => 
      item.label.toLowerCase().includes(query.toLowerCase())
    )
  })).filter(group => group.items.length > 0) : dynamicCommandGroups;
  ...
  const handleAction = (item: any) => {
    if (item.action === 'focus-task' && onFocusTask) {
      onFocusTask(item.taskData);
    } else if (item.tab) {
      onNavigate(item.tab);
    }
