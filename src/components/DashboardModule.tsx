...
import { StrategicBrainModule } from './StrategicBrainModule';
import { DailyStrategicInsight } from './DailyStrategicInsight';
...
export const DashboardModule = memo((props: DashboardModuleProps) => {
  ...
  const highPriorityAlertTasks = useMemo(() => {
    ...
    inThreeDays.setDate(today.getDate() + 2); // today and tomorrow
  ...
  const unifiedGraphData = useMemo(() => {
    ...
    if (props.meetings) {
      props.meetings.forEach(m => {
        data.push({
          id: `meeting-${m.id}`,
          title: `[Cuộc họp] ${m.name}`,
          content: m.description || m.name,
          category: 'Cuộc họp',
          tags: ['cuộc họp', 'hội nghị'],
          isImportant: m.priority === 'high'
        });
      });
    }

    // Add personnel (birthdays)
    if (props.birthdays) {
      props.birthdays.forEach(b => {
        data.push({
          id: `person-${b.id}`,
          title: `[Nhân sự] ${b.name}`,
          content: `Sinh nhật/Thông tin nhân sự: ${b.name}`,
    ...
    return data;
  }, [props.aiKnowledge, props.events, props.meetings, props.birthdays]);
  ...
  const confirmReseed = useCallback(async () => {
    setIsReseeding(true);
  ...
  return (
    ...
    >
      ...
      {highPriorityAlertTasks.length > 0 && (
        <motion.div 
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="relative overflow-hidden bg-gradient-to-r from-red-600 via-rose-600 to-amber-600 text-white rounded-[2rem] p-6 md:p-8 shadow-2xl shadow-rose-500/20 border border-white/10 flex flex-col md:flex-row md:items-center justify-between gap-6"
        >
          {/* Ambient Glows */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.15),transparent_60%)] pointer-events-none" />
          
          <div className="flex items-start gap-5 relative z-10">
            <div className="p-4 bg-white/15 backdrop-blur-md rounded-2xl text-amber-300 animate-pulse border border-white/20 shrink-0">
              <AlertTriangle size={28} />
            </div>
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <span className="px-2.5 py-0.5 bg-red-900/40 border border-red-500/30 text-[9px] font-black uppercase tracking-widest rounded-full">Ngữ cảnh thời gian thực</span>
                <span className="h-1.5 w-1.5 rounded-full bg-amber-400 animate-ping" />
              </div>
              <h4 className="text-lg md:text-xl font-black uppercase tracking-tight italic">PHÁT HIỆN NHIỆM VỤ ĐẾN HẠN</h4>
              <p className="text-sm font-medium text-white/95 max-w-2xl">
                Đồng chí có <strong className="text-amber-300 font-bold">{highPriorityAlertTasks.length} nhiệm vụ</strong> chưa hoàn thành sát ngày hoặc quá hạn deadline. Hệ thống khuyến nghị kích hoạt <strong className="text-amber-300 font-bold">Command Center Action</strong> để xử lý khẩn cấp.
              </p>
              
              {/* Most urgent task card */}
              <div className="mt-4 p-3.5 bg-black/20 rounded-xl border border-white/5 inline-flex items-center gap-3">
                <div className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse" />
                <span className="text-xs font-bold text-white/90">Nhiệm vụ:</span>
                <span className="text-xs font-black uppercase tracking-tight text-amber-300">{highPriorityAlertTasks[0].title}</span>
                <span className="text-white/40">|</span>
                <span className="text-xs font-bold text-white/80">Hạn chót:</span>
                <span className="text-xs font-bold bg-white/10 px-2 py-0.5 rounded text-white">{highPriorityAlertTasks[0].deadline}</span>
              </div>
            </div>
          </div>
          ...
          {props.onFocusTask && (
            <button
              onClick={() => props.onFocusTask && props.onFocusTask(highPriorityAlertTasks[0])}
              className="relative z-10 self-start md:self-center px-6 py-4 bg-white hover:bg-amber-50 text-red-700 hover:text-red-800 text-[11px] font-black uppercase tracking-[0.2em] rounded-2xl transition-all shadow-xl hover:shadow-2xl active:scale-95 flex items-center gap-3 shrink-0 border border-white"
            >
              <Rocket size={16} className="animate-bounce" />
              <span>Kích hoạt Action</span>
            </button>
          )}
        </motion.div>
      )}
      
      {/* Daily Strategic Insight Widget */}
      <div className="xl:col-span-12">
        <DailyStrategicInsight 
          tasks={props.tasks}
      ...
      <div className="xl:col-span-12">
        <KnowledgeGraph data={unifiedGraphData} />
      </div>
      ...
      <motion.div 
        ...
        initial="hidden"
        animate="show"
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-12 gap-8"
      >
        {/* Key Metrics Row */}
        ...
        <motion.div variants={itemVariants} className="xl:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <ConfirmationModal
        ...
        title="Nạp lại dữ liệu mặc định"
        message="Đồng chí có chắc chắn muốn nạp lại dữ liệu đánh giá và KPI mặc định? Thao tác này sẽ chỉ thêm các bản ghi dữ liệu mẫu còn thiếu để tối ưu hóa hiệu năng và tránh ghi đè dữ liệu tự tạo."
